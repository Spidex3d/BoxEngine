...
namespace ImGui
{
    ...
    IMGUI_API void          NewFrame();                                 // start a new Dear ImGui frame, you can submit any command from this point until RenderPreview()/EndFrame().
    IMGUI_API void          EndFrame();                                 // ends the Dear ImGui frame. automatically called by RenderPreview(). If you don't need to render data (skipping rendering) you may call EndFrame() without RenderPreview()... but you'll have wasted CPU already! If you don't need to render, better to not create any windows and not call NewFrame() at all!
    IMGUI_API void          RenderPreview();                                   // ends the Dear ImGui frame, finalize the draw data. You can then get call GetDrawData().
    IMGUI_API ImDrawData*   GetDrawData();                              // valid after RenderPreview() and until the next call to NewFrame(). this is what you have to render.
    ...
    IMGUI_API ImGuiMouseCursor GetMouseCursor();                                                // get desired mouse cursor shape. Important: reset in ImGui::NewFrame(), this is updated during the frame. valid before RenderPreview(). If you use software rendering by setting io.MouseDrawCursor ImGui will render those for you
...
struct ImGuiIO
{
    ...
    //------------------------------------------------------------------
    // Output - Updated by NewFrame() or EndFrame()/RenderPreview()
    ...
    int         MetricsRenderVertices;              // Vertices output during last call to RenderPreview()
    int         MetricsRenderIndices;               // Indices output during last call to RenderPreview() = number of triangles * 3
...
// RenderPreview state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing shader/blending settings before an Image call).
...
struct ImDrawList
{
    ...
    IMGUI_API void  PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max, bool intersect_with_current_clip_rect = false);  // RenderPreview-level scissoring. This is passed down to your render function but not used for CPU-side coarse clipping. Prefer using higher-level ImGui::PushClipRect() to affect logic (hit-testing and widget culling)
...
struct ImDrawData
{
    bool                Valid;              // Only valid after RenderPreview() is called and before the next NewFrame() is called.
...
struct ImGuiViewport
{
    ...
    ImDrawData*         DrawData;               // The ImDrawData corresponding to this viewport. Valid after RenderPreview() and until the next call to NewFrame().
...
// - Application:  In your main loop, call ImGui::UpdatePlatformWindows(), ImGui::RenderPlatformWindowsDefault() after EndFrame() or RenderPreview().
...
struct ImGuiPlatformIO
{
    ...
    // Viewports list (the list is updated by calling ImGui::EndFrame or ImGui::RenderPreview)
